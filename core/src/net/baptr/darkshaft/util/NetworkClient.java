...
import net.baptr.darkshaft.screen.DemoScreen;
import net.baptr.darkshaft.util.Network.*;
            ...
            new Listener() {
              ...
              public void received(Connection c, Object o) {
                ...
                if (o instanceof RemovePlayer) {
                  ...
                  if (p.id != myId) screen.removeRemoteAvatar(p.id);
                }
                if (o instanceof TowerPlaced) {
                  TowerPlaced t = (TowerPlaced) o;
                  if (t.player.id != myId) screen.addRemoteTower(t.player.id, t.type, t.col, t.row);
                }
                if (o instanceof Sync) {
                  Sync s = (Sync) o;
                  for (Player p : s.players) {
                    if (p.id != myId) screen.addRemoteAvatar(p.name, p.id, p.x, p.y);
                  }
                  for (TowerPlaced t : s.towers) {
                    screen.addRemoteTower(t.player.id, t.type, t.col, t.row);
                  }
                }
              }
            }));
